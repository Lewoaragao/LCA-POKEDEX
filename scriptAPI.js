const URL = "https://pokeapi.co/api/v2/pokemon?limit=100/"
let pesquisando = document.getElementById("pesquisando")
let dadosGerais = document.getElementById("dadosGerais")
let nomesPokemons = document.getElementById("nomesPokemons")
var estadoDados
var quantidadePokemons

window.onload = () => {
    pesquisarTodosPokemons()
}

async function pesquisarTodosPokemons() {
    try {
        const resposta = await fetch(URL)
        estadoDados = "Carregando dados..."

        const dados = await resposta.json()
        estadoDados = "Dados carregados."

        quantidadePokemons = dados.count

        dadosGerais.innerHTML = `
            <h2>Dados gerais</h2>
            <div class="row my-3">
                <div class="col list-group-item ">
                    <p class="d-flex justify-content-between align-items-center">Pokémons encontrados:</p>
                    <span class="badge bg-primary rounded-pill">${dados.count}</span>
                </div>
                <div class="col list-group-item ">
                    <p class="d-flex justify-content-between align-items-center">Estado dos dados:</p>
                    <span class="badge bg-primary rounded-pill">${estadoDados}</span>
                </div>
            </div>
        `

        // Populando opções do select de pokemons
        dados.results.forEach(pokemons => {

            let nomePokemon = pokemons.name
            let idPokemon = pokemons.url.replace("https://pokeapi.co/api/v2/pokemon/").replace(/[^0-9]/g, '')
            if (idPokemon < 10) {
                idPokemon = `000${idPokemon}`
            } else if (idPokemon < 100) {
                idPokemon = `00${idPokemon}`
            } else if (idPokemon < 1000) {
                idPokemon = `0${idPokemon}`
            }

            // nomesPokemons.innerHTML += `
            // <option value="${nomePokemon}">ID: ${idPokemon} - Nome: ${nomePokemon.toUpperCase()}</option>
            // `
        })

    }
    catch (error) {
        dadosGerais.innerHTML = `
                <h2>Dados gerais</h2>
                <div class="alert-danger my-3 p-2">Erro ao carregar dados!</div>
`
        console.log(error)
    }
}

function listItems(items, pageActual, limitItems) {
    let result = [];
    let totalPage = Math.ceil(items.length / limitItems);
    let count = (pageActual * limitItems) - limitItems;
    let delimiter = count + limitItems;
    if (pageActual <= totalPage) {
        for (let i = count; i < delimiter; i++) {
            if (items[i] != null) {
                result.push(items[i]);
            }
            count++;
        }
    }
    return result;
};
var next = ['Next 1', 'Next 2', 'Next 3', 'Next 4', 'Next 5'];
var resultNext = listItems(next, 1, 2);
var resultNext2 = listItems(next, 2, 2);
var resultNext3 = listItems(next, 3, 2);

console.log(resultNext)
